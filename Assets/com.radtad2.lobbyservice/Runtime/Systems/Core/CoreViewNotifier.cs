...
namespace LobbyService.Samples.Steam
{
    ...
    /// </summary>
    public class CoreViewNotifier : ICoreView, IChatView, IBrowserView, IDisposable
    {
        ...
        public CoreViewNotifier(LobbyController lobby)
        {
            _lobby = lobby;
            // TODO: Connect view
        }
        ...
        public void DisplayAddedDistanceFilter(LobbyDistance filter) => OnAddedDistanceFilter.Raise(filter);
        public void DisplayClearedDistanceFilter() => OnClearedDistanceFilter.Raise();

        public void DisplayAddedSorter(ILobbySorter sorter, string key) => OnAddedSorter.Raise(new LobbyKeyAndSorter
        {
            Sorter = sorter,
            Key = key
        });
